...
def state_to_features(game_state: dict) -> np.array:
    ...
    vision_coordinates = np.indices((reach*2+1, reach*2+1))
    ...
    if 0 <= min(min_coor) and all(max_coor<field.shape):
        ...
        wall_crates[abs(min(0,min_coor[0])):reach*2+2-abs(min(0, field.shape[0]-2-max_coor[0])), abs(min(0,min_coor[1])):reach*2+2-abs(min(0, field.shape[1]-2-max_coor[1]))] = field[max(0,min_coor[0]):min(max_coor[0]+1, field.shape[0]), max(0,min_coor[1]):min(max_coor[1]+1, field.shape[1])]
